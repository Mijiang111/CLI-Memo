...
export function renderAgentContextPrompt(bundle, verification = verifyAgentContextBundle("", bundle)) {
  ...
  return [
    ...
    "Dependency impact:",
    listLines((codeGraph.changedImpact || []).slice(0, 6), (item) => `- ${item.path}: dependents=${(item.dependents || []).slice(0, 3).join(", ") || "none"} dependencies=${(item.dependencies || []).slice(0, 3).join(", ") || "none"} tests=${(item.tests || []).slice(0, 3).join(", ") || "none"}`),
    "",
    "## Mission",
    "",
