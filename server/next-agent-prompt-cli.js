...
function usage() {
  return `Usage:
  node server/next-agent-prompt-cli.js --project-dir /path/to/project --role coding_agent --write
...
Options:
  ...
  --write               Write .project-agent/next-agent-prompt.md.
  --no-resume-write     Do not refresh .project-agent/resume.md.
...
const result = await buildNextAgentPrompt(projectDir, {
  ...
  writeFile: write,
  writeResume: !args.includes("--no-resume-write"),
